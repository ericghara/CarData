...
class GmScraper(ModelInfoScraper):
    ...
    def _getBodyStyleToName(self) -> dict:
        ...
        for year in range(date.today().year + 1, date.today().year - 2, -1):
            try:
                ...
                stylesAndNames = list()
                logging.debug(f'Unable to fetch model year {year}. More than 1 failure could indicate a problem')
    ...
    def _fetchBodyStyles(self, modelYear: 'date') -> Set[str]:
        ...
        try:
            ...
            logging.debug(f'Unable to fetch year: {modelYear.year} from shoppingLinks API.')
        ...
        try:
            ...
            logging.debug(f'Unable to fetch year: {modelYear.year} from getVehicleInfo API.')
